...
class KExpression2D(Expression):
	...
	def frack(self, X, Y):
		find = (np.abs(np.sqrt(pow((self.x_surf[:] - X), 2) + pow((self.y_surf[:] - Y), 2)))).argmin()
	...
	def eval(self, values, x, **kwargs):
		...
		if self.find_surface(x[0]) < 0.0:
			values[0] = pow(10, (self.log_kr + ((self.k_ocean - self.log_kr) * pow((1 - (1e-3 * (x[1] - self.find_surface(x[0])))), -self.alpha_k))))

		else:
			values[0] = pow(10, (self.log_kr + ((self.k_surf - self.log_kr) * pow((1 - (1e-3 * (x[1]))), -self.alpha_k))))
...
class KExpression3DConstant(Expression):
	...
	def eval(self, values, x, **kwargs):
		...
		if self.find_surface(x[0], x[1]) < 0.0:
			...
			values[0] = pow(10, (self.log_kr + ((self.k_surf - self.log_kr) * pow((1 - (1e-3 * (x[2]))), -self.alpha_k))))
...
class Poroelasticity2D:
    def __init__(self, data_file, ndim, mesh, boundaries, plot_figs, event, new_mesh, permeability, days, SSE_days,
        ...
        print "Running 2D model for a", event, " event with a slip of ", u0_SSE, " m/day: updated 11:50"
        self.name_append = "PS_test"
        ...
        self.kstick = 8e-4
        self.stick = Expression('1/(1+exp(-kstick*(1e5-x[0])))', kstick=self.kstick, degree=1)
        ...
        self.p_f = Expression('0.02*(sigv - ph)', sigv=self.sig_v, ph=self.p_hydrostatic, degree=1)
    ...
    def time_stepping(self):
        if self.event == 'EQ':  # 15 min for 1 day, 4 hrs for 20 days, 2 days for 30 days, per month for 10 years
            ...
            self.dtt = [15, 288e1, 432e2]  # in minutes
            self.ntime = [12, 10, 10]
    ...
    def k_frack(self, p):
        ...
        self.k_2D_func.vector()[self.dof_frack_new] *= 1e4
        self.k_2D_func.vector()[self.dof_frack_old[self.dof_unfrack_new]] *= 1e-2
        self.kappa = Expression('kappa', kappa=self.k_2D_func, degree=1)
    ...
    def assemble_system(self):
        ...
        self.a_K = assemble(((self.kappa) / self.muf) * inner(nabla_grad(self.p), nabla_grad(self.q)) * dx)  # Stiffness matrix
    ...
    def point_source(self, dt):
        ...
        coords_Q = self.Qspace.tabulate_dof_coordinates().reshape((self.Qspace.dim(), -1))
        bctest1 = DirichletBC(self.Qspace, (1), self.boundaries, 3)  # slab surface
        qtest = Function(self.Qspace)
        bctest1.apply(qtest.vector())
        dof_slabQ = np.where(qtest.vector() == 1)[0]

        dehydrate_depths = np.array([-40.0e3, -50.0e3]) #meters
        dehydrate_flux = dt*np.array([100.0, 10.0]) #m^3/sec * dt
        
        for i in range(0,len(dehydrate_depths)):
            dof = np.where(coords_Q[:,1] ==
                                 coords_Q[dof_slabQ[np.abs(coords_Q[dof_slabQ, 1] - dehydrate_depths[i]).argmin()],1])[0]
            PointSource(self.Qspace, Point(coords_Q[dof, 0][0], coords_Q[dof, 1][0]), dehydrate_flux[i]).apply(self.b)
    ...
    def solve_system(self):
        ...
        for i in range(self.nsteps):
            ...
            pnorm = Function(self.Qspace)
            pnorm.assign(project(p, self.Qspace, solver_type='mumps'))
            
            #print self.w0.vector().min(), self.w0.vector().max()
            #print self.w.vector().min(), self.w.vector().max()
            ...
            # ax1.set_title('Poroelastic Solution')


            # Ux, Uy = Sol_surf_x[:, 0], Sol_surf_y[:, 0]
            # x_surf_0, y_surf_0 = x_surf_u + scaleplot * Ux, Uy

            # Ux_post, Uy_post = Sol_surf_x[:, -1] - Sol_surf_x[:, 0], Sol_surf_y[:, -1] - Sol_surf_y[:, 0]
            # x_surf_post, y_surf_post = x_surf_0 + scaleplot * Ux_post, y_surf_0 + Uy_post
            
            # Co-seismic displacement
            # ax1.plot(self.Sol_surf, '-b*', ms = 2, mew = 2, fillstyle = 'none', color = 'blue',label = 'co-seismic land surface')


            # fig.savefig('figures/test.png', fmt = 'png', dpi = 200)
            ...
            #self.w00 = Function(self.ME)
            #self.w00.interpolate(self.w_init)
            #assign(self.w0, self.w00)
            #self.p00 = Function(self.Qspace)
            #self.p00.interpolate(self.p0)
            #assign(self.w0.sub(0), self.p00)
