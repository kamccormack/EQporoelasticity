...
def GPSlatlon2XY(data_sheet, origin, theta):
	...
	return 1e3*X, 1e3*Y, 1e-2*Ux, 1e-2*Uy, 1e-2*Uz
...
#Sol_gps_k = np.load(results_path+"Sol_gps_3D.npy")
#Sol_surf_k = np.load(results_path+"Sol_surf_3D.npy")
...
dtt = [5256e2, 15, 240, 288e1, 432e2]#, 5256e2]  # in minutes
ntime = [50, 96, 120, 15, 122]#, 50]
...
def plot_gps(Sol_gps, size_gps, n):
...
def plot_gps_surface():
	...
	quiver = ax1.quiver(X_gps/1e3, Y_gps/1e3, Ux_gps, Uy_gps, color = 'blue', label='GPS data',units='inches',scale_units='width', width=5e-2)#, scale=1e1,)
	quiver2 = ax1.quiver(X_gps/1e3, Y_gps/1e3, Ux_gps_model, Uy_gps_model, color = 'red', label='model', units='inches',scale_units='width', width=5e-2)#, scale=1,)
