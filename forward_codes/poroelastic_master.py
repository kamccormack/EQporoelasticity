...
class Poroelasticity2D:
	...
	def slip_interpolation(self):
		...
		bctest = DirichletBC(self.Qspace, (3), self.boundaries, 3)  # slab surface
...
class Poroelasticity3DSynthetic:
	def __init__(self, data_file, origin, theta, ndim, mesh, boundaries, plot_figs, event, new_mesh, permeability,
		...
		self.x_gps = np.array(
			[65308.5, 78441.2, 65707.9, 73555.6, 61257.8, 65291.6, 82112.3, 58804.1, 106822.8, 60152.4, 144960.9,
			 88660.1, 134829.2, 82409, 65214.4, 67249.7, 91916.7, 111195.9, 92905.4, 77176.5, 90809.3, 107806.1,
			 104439.5, 137051.8, 118095, 82238.4, 84123.2, 98197.5, 105163, 154799.8, 133655.9, 111467, 125498.1,
			 106995.1, 149450.7, 192061, 133903.2])

		self.y_gps = np.array(
			[173604.8, 185529.2, 168017.6, 174095.8, 157386.9, 154352.5, 172814.3, 138685.6, 190893.8, 136426.2,
			 230041.9, 163931.9, 208246.3, 149993.8, 127836, 122418.9, 141284.3, 162013.4, 131283.2, 109747.4, 124059.5,
			 141615.9, 126126.3, 160541, 132800.4, 90851.5, 92291, 105309.4, 104713.8, 153672.9, 122823.1, 94557.3,
			 99698.5, 77971.8, 123239.8, 157764, 45232])
...
class Laplace2Dsteady:
	def __init__(self, data_file, origin, theta, ndim, mesh, boundaries, plot_figs, event, new_mesh, permeability,
	             sub_cycle_years, surface_k, ocean_k, loop, i, **kwargs):

		self.txt_append = "_flat"
		self.name_append = self.txt_append + str(i)
		print self.name_append
		...
		# self.prm["form_compiler"]["representation"] = "uflacs"
		# self.prm['reuse_factorization'] = True  # Saves Factorization of LHS
		self.Qelement = FiniteElement('CG', self.mesh.ufl_cell(), 2)
		...
		pfilename = self.paraviewpath + "pressure2D" + self.name_append + ".pvd"
		p_velfilename = self.paraviewpath + "flux0_comp2D" + self.name_append + ".pvd"
		p_velfilename1 = self.paraviewpath + "flux1_comp2D" + self.name_append + ".pvd"
	...
	def solve_system(self):
		...
		self.bound_flux2 = assemble(inner(self.domain_flux, self.n) * self.ds(2))
		self.bound_fluxall = assemble(inner(self.domain_flux, self.n) * self.ds)
		flux_print = [self.bound_flux, self.bound_flux2, self.bound_fluxall]

		with open("topo" + self.txt_append + "_flux.txt", "a") as f:
			for flux in flux_print:
				f.write(str(flux) + ", ")
			f.write('\n')
		...
		# self.psi = self.streamfunction(q_comp1)  # compute numerical streamfunction
...
class Laplace3Dsteady:
	...
	def topo_flow_interp(self):
		...
		self.coords_Q = self.Q.tabulate_dof_coordinates().reshape((self.Q.dim(), -1))
		bctest1 = DirichletBC(self.Q, (1), self.boundaries, 1)  # ocean surface
		bctest2 = DirichletBC(self.Q, (1), self.boundaries, 2)  # land surface
		qtest = Function(self.Q)
		bctest1.apply(qtest.vector())
		bctest2.apply(qtest.vector())
		self.dof_surfQ = np.where(qtest.vector() == 1)[0]
		...
		#                          ' + (pow((x[1] - ycenter),2)/(2*pow(sigmay,2)))))'), pmax = topo_max,
		#                         xcenter = xcenter, ycenter = ycenter,sigmax = sigma_bx, sigmay = sigma_by, degree = 1)

		self.p_topo = HtopoExpression3D(topo_data, self.origin, self.theta,
		                                self.coords_Q, self.dof_surfQ, self.var_path, degree=1)
