...
ind_2dcross = np.where(Y_gps_post < 170e3)[0]
ind_2dpost = np.where(Y_gps_post[ind_2dcross] > 110e3)[0]
...
if event == 'EQ':
	if loop == 'yes':
		if variable == 'kappa':
			...
			# Sol_all_k11 = np.load(resultspath + "Sol_all_2D_EQ_loop_k11.npy")
			# Sol_all_k12 = np.load(resultspath + "Sol_all_2D_EQ_loop_k12.npy")
			# Sol_all_k13 = np.load(resultspath + "Sol_all_2D_EQ_loop_k13.npy")
...
surface_dof_ux = surface_dofs[0:ux_ind]#[np.argsort(x_all[surface_dofs[0:ux_ind]])]
surface_dof_uy = surface_dofs[ux_ind::]#[np.argsort(x_all[surface_dofs[ux_ind::]])]
sort_u = np.argsort(x_all[surface_dofs[ux_ind::]])
...
if loop == 'yes':
	if variable == 'kappa':
		...
		#
		Sol_surf_xk10 = Sol_surfk10[0:ux_ind, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_xk11 = Sol_surf[0:ux_ind, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_xk12 = Sol_surfk12[0:ux_ind, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_xk13 = Sol_surfk13[0:ux_ind, :]* 1e2#[sort_u, :] * 1e2
	#
		Sol_surf_yk10 = Sol_surfk10[ux_ind::, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_yk11 = Sol_surf[ux_ind::, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_yk12 = Sol_surfk12[ux_ind::, :]* 1e2#[sort_u, :] * 1e2
		Sol_surf_yk13 = Sol_surfk13[ux_ind::, :]* 1e2#[sort_u, :] * 1e2
...
def plot_timeseries_head(points):
	...
	for i in range(0, points.shape[0]):
		...
		end  = np.where(dttnew/30. > 24.)[0][0]



		gps_time = data[names[ind_2D_time[i]] + '_time']
		lon_time = data[names[ind_2D_time[i]] + '_lon']
		lat_time = data[names[ind_2D_time[i]] + '_lat']
		Uz_time = data[names[ind_2D_time[i]] + '_rad']
		#
		Ux_time, Uy_time = GPSlatlon2XY_time(lat_time, lon_time, theta)
		...
		Ux_post += Sol_surf_xk10[idx_u, 0] - Ux_post[0]
		Uz_post += Sol_surf_yk10[idx_u, 0] - Uz_post[0]
		...
		ax2.plot(dttnew[0:end]/days, Sol_surf_xk10[idx_u, 0:end], 'red', label='log(k) = -10',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none', color = 'pink') #pressure
		ax2.plot(dttnew[0:end]/days, Sol_surf_xk11[idx_u, 0:end], 'k', label='log(k) = -11',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
		ax2.plot(dttnew[0:end]/days, Sol_surf_xk12[idx_u, 0:end], 'lightblue', label='log(k) = -12',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none', color = 'lightblue') #pressure
		ax2.plot(dttnew[0:end]/days, Sol_surf_xk13[idx_u, 0:end], 'b', label='log(k) = -13',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
		ax2.plot(time_post, Ux_post, '.k', linewidth=3, fillstyle='none')
		
		ax3.plot(dttnew[0:end]/days, Sol_surf_yk10[idx_u, 0:end], 'red', label='log(k) = -10',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none', color = 'pink') #pressure
		ax3.plot(dttnew[0:end]/days, Sol_surf_yk11[idx_u, 0:end], 'k', label='log(k) = -11',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
		ax3.plot(dttnew[0:end]/days, Sol_surf_yk12[idx_u, 0:end], 'lightblue', label='log(k) = -12',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none', color = 'lightblue') #pressure
		ax3.plot(dttnew[0:end]/days, Sol_surf_yk13[idx_u, 0:end], 'b', label='log(k) = -13',
		         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
		ax3.plot(time_post, Uz_post, '.k', linewidth=3, fillstyle='none')
		...
		#ax2.set_ylim([Ux_post.min()-2. , Ux_post.max()+2.])
		#ax3.set_ylim([Uz_post.min()-2. , Uz_post.max()+2.])
		...
		fig.savefig('figures/2D_timeseriesRT_%s.png' % str(points[i]), dpi=400)
...
def plot_topo():
	...
	ax1.scatter(x_surf_ux, Sol_surf_xk10[:,0])
	ax2.scatter(x_surf_uy, Sol_surf_yk10[:,0])
	ax1.plot(1e-3*X_gps_post[ind_2D_time], Ux_cosies, '.r')
	ax2.plot(1e-3*X_gps_post[ind_2D_time], Uz_cosies, '.r')
	...
	fig.savefig('figures/xsurf_test.png', dpi=400)
